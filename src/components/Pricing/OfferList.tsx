...
const OfferList = ({
  text,
  status,
  heading = false,  // Add heading prop, default is false
}: {
  text: string;
  status?: "active" | "inactive"; // Make status optional
  heading?: boolean;  // Optional heading prop
}) => {
  const leftMarginClass = heading && !status ? "ml-8" : "";
  return (
    <div className={`mb-3 flex items-center ${leftMarginClass}`}>
  {/* If it's a heading, apply different styles */}
  {heading ? (
    <>
      {/* Display heading text with optional status */}
      {status && (
      ...
      <h3 className="text-lg font-light text-black dark:text-white">{text}</h3>  {/* Heading text */}
    </>
  ) : (
    <>
      {/* Display status icon for non-heading items */}
      {status && (
        <span className="mr-3 flex h-[18px] w-full max-w-[18px] items-center justify-center rounded-full bg-primary bg-opacity-10 text-primary">
          {status === "active" ? checkIcon : crossIcon}
        </span>
